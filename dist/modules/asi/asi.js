...
const ASI = (parser, error) => {
    const { token, previousToken, chart } = error;
    const { lexer } = parser;
    const optSemi = chart.lastColumn?.states.find(state => state.lhs === 'OptSemi' && !state.isComplete);
    if (token) {
        if (!previousToken) {
        ...
        else if (previousToken.name === Tokens.NEWLINE ||
        ...
        else if ((previousToken.name === Tokens.LCBRACE || previousToken.name === Tokens.SEMI) &&
            token.name === Tokens.NEWLINE) {
            return true;
        }
        else {
            throw SyntaxError(`Unexpected token ${previousToken.value} (${previousToken.line}:${previousToken.col})`);
        }
    }
    else {
